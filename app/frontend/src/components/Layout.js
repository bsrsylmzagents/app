...
const Layout = () => {
  ...
  const getIconColor = (isActive) => {
    if (isDynamicTheme) {
      // All icons are white in dynamic theme
      return 'text-white';
    }
          ...
          <nav className="flex-1 px-4 md:px-6 py-6 overflow-y-auto">
            <ul className="space-y-3">
              ...
              {adminMenuItems.length > 0 && (
                <li className="py-2">
                  <div className={`h-px ${isDynamicTheme ? 'bg-white/20' : 'bg-border'}`}></div>
                </li>
              )}
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-colors ${
                  isDynamicTheme 
                    ...
                    : 'bg-[#3EA6FF]/20 hover:bg-[#3EA6FF]/30 text-[#3EA6FF]'
                }`}
            ...
            <button
              ...
              className={`relative p-2 rounded-lg transition-colors ${
                isDynamicTheme 
                  ...
                  : 'bg-[#3EA6FF]/10 border border-[#3EA6FF]/30 hover:bg-[#3EA6FF]/20'
              }`}
              title="Bildirimler"
            >
              <BellPhosphor weight="duotone" size={20} className={isDynamicTheme ? 'text-white' : 'text-[#3EA6FF]'} />
              {unreadCount > 0 && (
